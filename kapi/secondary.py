"""Implementation-isolated recalculation for frozen KAPI calculations.

This implementation intentionally does not import the primary calculation
engine. It recomputes provider/creator-diverse selections, medians, basket costs
and index levels from the ledger embedded in the frozen calculation artifact.
It is a technical cross-check, not a human external review.
"""
...
class SecondaryCheckError(ValueError):
    """Raised when the frozen calculation lacks recalculable ledger data."""


def _decimal(value: Any, label: str) -> Decimal:
    if isinstance(value, float) or isinstance(value, bool):
        raise SecondaryCheckError(f"{label} is not an exact decimal")
    try:
        result = Decimal(str(value))
    except (InvalidOperation, ValueError) as error:
        raise SecondaryCheckError(f"{label} is not an exact decimal") from error
    if not result.is_finite():
        raise SecondaryCheckError(f"{label} is not finite")
    return result
...
def _best_diverse_triple(
    ...
    if not triples:
        raise SecondaryCheckError(
            "candidate ledger has no three-provider/three-creator diverse triple"
        )
...
def _check_calculation(calculation: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(weeks, list) or not isinstance(weights, Mapping) or not isinstance(base, Mapping):
        raise SecondaryCheckError("calculation is missing weeks, weights, or base_period")
    ...
    for week in weeks:
        ...
        if not isinstance(profiles, list):
            raise SecondaryCheckError(f"week {week_id} has no profile ledger")
        basket = Decimal(0)
        complete = True
        ...
        for profile in profiles:
            ...
            selected_ids, selected_total, median = _best_diverse_triple(candidates)
    ...
    if base_status == "complete":
        ...
        if not isinstance(base_ids, list) or not base_ids:
            raise SecondaryCheckError("complete base has no week IDs")
        base_baskets = []
        for week_id in base_ids:
            entry = recomputed.get(str(week_id))
            if not entry or not entry["complete"]:
                raise SecondaryCheckError("base references an incomplete recomputation")
    ...
    return {
        "implementation": "kapi.implementation_isolated_recalculation.v2",
        "implementation_isolated_from_primary_module": True,
        "human_external_review": False,
        "claim_scope": "implementation-isolated recalculation",
...
def check_secondary_calculation(calculation: Mapping[str, Any]) -> dict[str, Any]:
    """Return a deterministic implementation-isolated comparison report."""
